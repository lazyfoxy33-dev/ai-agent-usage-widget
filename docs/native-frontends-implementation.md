...
- Python data layer: 74 tests.
- Python 数据层：74 项测试。
- Existing Übersicht frontend: 9 tests.
- 现有 Übersicht 前端：9 项测试。
...
- Windows web UI: 2 Node tests.
- Windows Web UI：2 项 Node 测试。
- Tauri Rust backend: 3 tests and host `cargo check`.
- Tauri Rust 后端：3 项测试及宿主机 `cargo check`。
...
These items are deliberately reported as pending because neither Apple signing
credentials nor a Windows host can be simulated reliably by the current macOS
build environment.

以上项目保持“待验收”，因为当前 macOS 构建环境无法可靠模拟用户的 Apple 签名
凭据或 Windows 真机。
