...
The Kimi panel uses the same official endpoint as Kimi Code CLI's interactive
`/usage` command:
...
The widget reads the OAuth token from the current Kimi Code CLI location, then
falls back to the legacy Kimi CLI location:
...
组件优先读取当前 Kimi Code CLI 路径，再兼容旧 Kimi CLI 路径。它是只读使用方：
...
- Kimi Code CLI's official black granular `K` logo
...
- The official Kimi Code CLI logo from `MoonshotAI/kimi-code`
...
仓库内保存 Kimi Code CLI 官方 Logo 和 Codex App 完整云朵图标，由安装脚本复制到组件
...
- Kimi credential path resolution, including `KIMI_CODE_HOME` and legacy
  `KIMI_SHARE_DIR`
