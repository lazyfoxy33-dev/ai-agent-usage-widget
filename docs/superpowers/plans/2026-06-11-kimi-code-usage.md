...
Kimi CLI OAuth file and calls the same `/coding/v1/usages` endpoint as Kimi
CLI. `fetch_usage.py` owns five-minute caching and stale fallback. `index.jsx`
renders the third provider using local brand assets copied by `install.sh`.
...
def credentials_path():
    share = os.environ.get("KIMI_SHARE_DIR") or os.path.expanduser("~/.kimi")
    return os.path.join(share, "credentials", "kimi-code.json")
...
- Kimi: `MoonshotAI/kimi-cli/web/public/logo.png`
